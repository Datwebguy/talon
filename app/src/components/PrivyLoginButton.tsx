...
export function PrivyLoginButton({ onSuccess }: { onSuccess?: () => void }) {
  ...
  return (
    <button
      onClick={handleLogin}
      className="w-full p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-[#010FEE]/10 to-indigo-500/10 hover:from-[#010FEE]/20 hover:to-indigo-500/20 border border-[#010FEE]/30 hover:border-[#010FEE] transition-all flex items-center justify-between group text-left shadow-sm mb-3"
    >
      <div className="flex items-center gap-3.5">
        ...
        <div>
          ...
          <div className="text-[11px] text-[#64748B] dark:text-[#94A3B8] font-medium mt-0.5">
            Google, Apple, SMS, or Passkey (Privy)
          </div>
        </div>
      </div>
